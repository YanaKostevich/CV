...
const data = {
    ...
    courses: [
      {
        ...
        qualification: `<a href="https://www.sololearn.com/en/certificates/CC-KINQBFNK">Introduction to JavaScript</a>`,
        period: ["2023", "now"],
      },
    ],
  },
  ...
  makeMyStory() {
    return this.about
      ? `<h2>A few words about me</h2> <p>${this.about
          .map((item) => `${item} `)
          .join(" ")}</p>`
      : "";
  },
  ...
  makeLanguages() {
    return this.languages
      .map(
        (item) =>
          `<span class="lang-item">${item.name}<s>${item.level}</s></span>`
      )
      .join("");
  },
  makeSkills() {
      ...
      .map(
        (item) =>
          `<span class="skill-item">${
            item.name
          }<s>${item.experience.toString()}</s></span>`
      )
      .join("");
  },
  makeInterests() {
    return this.interests
      .map((item) => `<span class="hobby-item">${item}</span>`)
      .join("");
  },
  makeWorkExperience() {
    let experienceItems = "";
    ...
    sortedItems.forEach((item) => {
      experienceItems += `<h4>${item.companyName} <span>${item.period[0]} - ${
        item.workNow === true ? "now" : item.period[1]
      }</span></h4>
                                <h5>${item.position}</h5>
  ...
  makeEducation() {
    ...
    let additional = `${
      this.education.additional
        ? `<h3>Additional</h3> ${this.education.additional
            .map(
              (item) =>
                `<div class="eduItem">
                    <h4>${item.name} <span>${item.period[0]} - ${item.period[1]}</span></h4>
  ...
  makeEducationCourses() {
    let eduRow = `<h3>Courses</h3>`;
    let sortedItems = this.educationCourse.courses.sort(function (a, b) {
      if (a.period[1] < b.period[1]) {
        return 1;
      }
      if (a.period[1] > b.period[1]) {
    ...
    let additional = `${
      this.educationCourse.additional
        ? `<h3>Additional</h3> ${this.educationCourse.additional
            .map(
              (item) =>
                `<div class="eduItem">
                    <h4>${item.name} <span>${item.period[0]} - ${item.period[1]}</span></h4>
